...
public class EasyControlsButton : ForgeEditorDisplayButton
{
	...
	public ushort PositionBits, RotationBits, ScaleBits;
	...
	public void Initialize(bool lerpPosition, 
		...
		ushort serializedPositionBits, 
		ushort serializedRotationBits,
		ushort serializedScaleBits)
	{
		...
		PositionBits = serializedPositionBits;
		RotationBits = serializedRotationBits;
		ScaleBits = serializedScaleBits;
	...
	public override void UpdateBits(ushort position, ushort rotation, ushort scale)
	{
	...
	public override void Update(Event windowEvent, GUIStyle fontStyle, Editor window, int width, int height)
	{
		...
		Rect lerpPositionBitsField = new Rect(original);
		lerpPositionBitsField.xMin += 40;
		lerpPositionBitsField.xMax -= (width - 60);
		lerpPositionBitsField.yMin += 55 + ADJUSTABLE_HEIGHT_MIN;
		lerpPositionBitsField.yMax -= 90 + ADJUSTABLE_HEIGHT_MAX;

		Rect lerpRotationBitsField = new Rect(original);
		lerpRotationBitsField.xMin += 40;
		lerpRotationBitsField.xMax -= (width - 60);
		lerpRotationBitsField.yMin += 95 + ADJUSTABLE_HEIGHT_MIN - 10;
		lerpRotationBitsField.yMax -= 50 + ADJUSTABLE_HEIGHT_MAX + 10;

		Rect lerpScaleBitsField = new Rect(original);
		lerpScaleBitsField.xMin += 40;
		lerpScaleBitsField.xMax -= (width - 60);
		lerpScaleBitsField.yMin += 135 + ADJUSTABLE_HEIGHT_MIN - 20;
		lerpScaleBitsField.yMax -= 10 + ADJUSTABLE_HEIGHT_MAX + 20;
		...
		if (_submitButtonVisibile)
		{
			Rect submitField = new Rect(original);
			submitField.xMin += 10;
			submitField.xMax -= (width - (width * 0.50f));
			submitField.yMin += 20 + _submitButtonHeight;
			submitField.yMax -= 125 - _submitButtonHeight - 15;

			Rect disableField = new Rect(original);
			disableField.xMin += (width - (width * 0.5f));
			disableField.xMax -= (width - (width * 0.96f));
			disableField.yMin += 20 + _submitButtonHeight;
			disableField.yMax -= 125 - _submitButtonHeight - 15;
			...
			GUI.color = Color.white;
			if (GUI.Button(disableField, "Disable"))
			{
				_easyControlsButtonVisible = false;
				LerpPosition = false;
				LerpScale = false;
				LerpRotation = false;
				_lerpPosX = false;
				...
				RotationBits = 0;
				ScaleBits = 0;
				SerializePosition = NetworkedMonoBehavior.SerializeVector3Properties.None;
				SerializeRotation = NetworkedMonoBehavior.SerializeVector3Properties.None;
				SerializeScale = NetworkedMonoBehavior.SerializeVector3Properties.None;
			}
			...
			//GUI.Label(lerpPositionBitsField, PositionBits + " Bits", fontStyle);
			//GUI.Label(lerpRotationBitsField, RotationBits + " Bits", fontStyle);
			//GUI.Label(lerpScaleBitsField, ScaleBits + " Bits", fontStyle);
